...
func show_dialog():
    match Inventory.astronomer:
        Inventory.VipState.MISSING:
            await DialogManager.show_dialog("We picked up a distress signal from our team lead in s0-4. Can you please rescue them?")
        Inventory.VipState.FOUND:
            ...
            await DialogManager.show_dialog("If we get enough data from them, perhaps we can improve ship sensors...")
